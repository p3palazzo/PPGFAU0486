---
title: 'Arquitetura adequada ao lugar <span class="subtitle">uma perspectiva histórica</span>'
date: 2023-08-31
authors:
  - Pedro Palazzo
  - Elane Peixoto
author_profile: true
layout: single
toc: true

fundamentos:
  - title: "Substrato urbanístico"
    excerpt: |
      Os quatro paradigmas da urbanização tradicional – Parcelamento
      do solo e bioclimatismo – Regimes tradicionais de propriedade
      fundiária

      - Trindade 2013
      - Dovey 2015
    image_path: https://palazzo.arq.br/TAU0009/assets/media/320px-Livro_das_Fortalezas_25_Elvas.jpg
    url       : "#"
    btn_label : "31 de agosto"
    btn_class : "btn btn--inverse btn--small"

  - title: "Feriado"
    image_path: /assets/media/320px-Pedro_Americo-Independencia_ou_Morte-crop.jpg
    url       : "#"
    btn_label : "7 de setembro"
    btn_class : "btn btn--inverse btn--small"

  - title: "O que é « lugar »"
    excerpt: |
      Introdução ao estudo do lugar – O passado dos lugares não é um
      país estrangeiro – Princípios de topofilia e fenomenologia

      - Tuan 1980
      - Pallasmaä
    image_path: https://i.pinimg.com/236x/83/69/d9/8369d9524b7b0bebbd453bc5c853281e.jpg
    url       : "#"
    btn_label : "14 de setembro"
    btn_class : "btn btn--inverse btn--small"

  - title: "Agentes e lugares"
    excerpt: |
      Arquitetos calçados e descalços – Percursos formativos e
      vinculação dos artesãos aos lugares – Persistências
      tradicionais em meio à modernização

      - Bueno 2011
      - Palazzo 2021
    image_path: https://palazzo.arq.br/TAU0009/assets/media/320px-mapa-comarca-sabara-arpmg-747.jpg
    url       : "#"
    btn_label : "21 de setembro"
    btn_class : "btn btn--inverse btn--small"

  - title: "Construção vernacular"
    excerpt: |
      Vinculação das tradições aos lugares – Sistemas
      construtivos – Processos produtivos artesanais e
      impacto da indústria na transmissão de saberes e práticas

      - Oliver 2006
      - Vellinga 2006
    image_path: https://palazzo.arq.br/TAU0009/assets/media/320px-debret-scieurs-nypl.jpg
    url       : "#"
    btn_label : "5 de outubro"
    btn_class : "btn btn--inverse btn--small"

  - title: "Feriado"
    image_path: /assets/media/320px-Igreja_do_Santíssimo_Sacramento_de_SantAna_Salvador_Nossa_Senhora_de_Aparecida_2018-0471.jpg
    url       : "#"
    btn_label : "12 de outubro"
    btn_class : "btn btn--inverse btn--small"

tipos:
  - title: "A casa e a oficina"
    excerpt: |
      Visão holística da arquitetura vernácula – Tipologia e
      Morfologia – Estilos como síntese dos saberes vernaculares –
      O cômodo como unidade mínima da identidade local

      - Rapoport 1972
      - Aragão 2017
      - Palazzo 2018
    image_path: https://palazzo.arq.br/TAU0009/assets/media/320px-br-pe-recife-r_cruz-schlappriz-carls-1863.jpg
    url       : "#"
    btn_label : "19 de outubro"
    btn_class : "btn btn--inverse btn--small"

  - title: "Processos e padrões"
    excerpt: |
      Tradições locais como articulação de padrões – Caráter e
      funcionalidade – Espaços e limiares

      - Alexander _et al._ 2012
      - Guizzo 2014
      - Malta & Mendonça 2013
    image_path: https://palazzo.arq.br/TAU0009/assets/media/320px-Frans_Post-Een_suikerfabriek-2116-Museum_Boijmans_Van_Beuningen.jpg
    url       : "#"
    btn_label : "26 de outubro"
    btn_class : "btn btn--inverse btn--small"

  - title: "Feriado"
    image_path: /assets/media/320px-debret-divers-convois-funebres-pl030.jpg
    url       : "#"
    btn_label : "2 de novembro"
    btn_class : "btn btn--inverse btn--small"

  - title: "Ecossistema construído"
    excerpt: |
      Interações entre geografia, geologia, bioma, clima e arquitetura
      – Respostas culturais às condicionantes naturais – Impactos
      ecológicos da produção do ambiente construído

      - Souza 2016
      - Simões _et al._ 2019
      - Bustos Romero 2001
      - Capitel 2005
    image_path: https://palazzo.arq.br/TAU0009/assets/media/320px-in-goa-dandim-1886-indiaportuguezab02lope_0272.jpg
    url       : "#"
    btn_label : "9 de novembro"
    btn_class : "btn btn--inverse btn--small"

  - title: "Tecnologias do lugar"
    excerpt: |
      Processos produtivos e tecnologias sociais – Simulações
      bioclimáticas e estruturais no estudo das tradições locais –
      Patrimônio cultural e valor social

      - Fernandes 2015
      - Correia _et al._ 2014, 22–84
    image_path: /assets/media/320px-br-sp-sp-r_rosario-jwr-1920.jpg
    url       : "#"
    btn_label : "16 de novembro"
    btn_class : "btn btn--inverse btn--small"

linguagens:
  - title: "Estilo e identidade"
    excerpt: |
      Estilo como metonímia da identidade local – Formação de
      linguagens arquitetônicas – Tratadística erudita e saberes
      vernaculares

      - Krier 2009
      - Adam 2019
    image_path: https://palazzo.arq.br/TAU0009/assets/media/320px-br-ba-ssa-piedade-victor_frond-louis_aubrun-1858.jpg
    url       : "#"
    btn_label : "23 de novembro"
    btn_class : "btn btn--inverse btn--small"

  - title: "Identidades coletivas"
    excerpt: |
      Identidade como construção interpessoal – Participação
      social na produção do ambiente construído – Tradições e
      coesão social

      - Cenicacelaya 2014
      - Morin 2000
    image_path: https://palazzo.arq.br/TAU0009/assets/media/320px-br-rj-rj-paco-imperial-willmann-1867.jpg
    url       : "#"
    btn_label : "30 de novembro"
    btn_class : "btn btn--inverse btn--small"

  - title: "Fazer vernacular"
    excerpt: |
      Autenticidade do vernáculo contemporâneo – Teorias unificadas
      da arquitetura – Regionalismo crítico e tradicionalismo
      figurativo

      - Fathy 1982
      - Frampton 1985
      - Salingaros 2013
    image_path: https://palazzo.arq.br/TAU0009/assets/media/320px-ranzini-estilos-colonial-pt-1929.jpg
    url       : "#"
    btn_label : "7 de dezembro"
    btn_class : "btn btn--inverse btn--small"

  - title: "Tema livre"
    excerpt: >
      Abordagem de tópicos de interesse da turma
    image_path: https://palazzo.arq.br/TAU0009/assets/media/320px-Antonio_Ferrigno-Beneficiamento-Museu_Paulista.jpg
    url       : "#"
    btn_label : "14 de dezembro"
    btn_class : "btn btn--inverse btn--small"

header:
  overlay_image: https://upload.wikimedia.org/wikipedia/commons/thumb/c/cd/José_Wasth_Rodrigues_-_Rua_do_Rosário,_1858,_Acervo_do_Museu_Paulista_da_USP.jpg/2560px-José_Wasth_Rodrigues_-_Rua_do_Rosário,_1858,_Acervo_do_Museu_Paulista_da_USP.jpg
  overlay_filter: 0.5
  caption: "José Wasth Rodrigues, [Rua do Rosário em 1858](https://commons.wikimedia.org/wiki/File:José_Wasth_Rodrigues_-_Rua_do_Rosário,_1858,_Acervo_do_Museu_Paulista_da_USP.jpg), 1920"

sidebar:
  - title: Expediente
    text: |
      - 60 horas presenciais
      - Quinta 17h–20h40
      - Sala de reuniões do mezanino Norte

tagline: |
  <dl>
    <dt>Ementa</dt>
    <dd>
      História das respostas às solicitações climáticas. O lugar e
      a perspectiva histórica do traçado das cidades, dos edifícios
      e dos espaços públicos externos.
    </dd>
    <dt>Objetivo de aprendizagem</dt>
    <dd>
      Ser capaz de integrar à própria pesquisa uma reflexão crítica
      acerca dos significados sociais e das identidades culturais
      atinentes à adaptação do ambiente construído às
      condicionantes ambientais e históricas da sua localidade.
    </dd>
  </dl>
---

# Unidade I › Fundamentos #

```{=html}
{% include feature_row id="fundamentos" %}
```

# Unidade II › Tipos #

```{=html}
{% include feature_row id="tipos" %}
```

# Unidade III › Linguagens #

```{=html}
{% include feature_row id="linguagens" %}
```

# Metodologia #

Esta disciplina de pós-graduação será integrada verticalmente com a
turma noturna da disciplina de graduação [TAU 0009]{.smallcaps}
Arquitetura e Urbanismo no Brasil Colônia e Império. A primeira metade
do horário, das 17h às 18h40, será exclusiva para a pós-graduação, e a
segunda metade, das 19h às 20h40, será realizada conjuntamente com as
duas turmas.

A disciplina será conduzida por meio da discussão de textos propostos
pela equipe docente e da análise de lugares relevantes para as pesquisas
discentes.

# Avaliação #

O desempenho na disciplina será aferido pela elaboração de um catálogo
de práticas e saberes vernaculares, afeito ao objetivo de aprendizagem e
ao programa da disciplina, em tema e formato a ser pactuado entre cada
estudante e a equipe docente.

# Bibliografia básica #

```{=html}
{% include biblio-basica.html %}
```

# Bibliografia complementar #

```{=html}
{% include biblio-complementar.html %}
```

